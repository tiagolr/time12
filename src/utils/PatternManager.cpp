...
{
    auto fileChooser = std::make_unique<juce::FileChooser>(exportWindowTitle,
    ...
    fileChooser->launchAsync(flags, [fileChooserPtr = fileChooser.release(), patterns, sequencer](const juce::FileChooser& fc)
    {
        std::unique_ptr<juce::FileChooser> scopedChooser(fileChooserPtr);
